...
async function run() {
  ...
  const neighbors = getNeighborsFromDictionary(
    "cat",
    dictionaryByLength,
    new Set(["cat"]),
  );
  ...
  const { dictionaryByLength: dict2, dictionarySet: set2 } =
    preprocessDictionary(words2);
  const chain3 = findChain("a", "ba", dict2, set2);
  assert.deepStrictEqual(chain3, ["a", "ba"]);

  const words3 = ["ab", "a", "b"];
  const { dictionaryByLength: dict3, dictionarySet: set3 } =
    preprocessDictionary(words3);
  const chain4 = findChain("ab", "b", dict3, set3);
  assert.deepStrictEqual(chain4, ["ab", "b"]);

  const words4 = ["ab", "a", "ca", "cd"];
  const { dictionaryByLength: dict4, dictionarySet: set4 } =
    preprocessDictionary(words4);
  ...
  assert.strictEqual(
    html,
    '<ol class="chain"><li>a</li><li><span class="diff">b</span></li></ol>',
  );

  // ensure dictionary words are normalized to lowercase
  const mixedWords = ["Cat", "cot", "cog", "Dog"];
  const { dictionaryByLength: mixedDict, dictionarySet: mixedSet } =
    preprocessDictionary(mixedWords);
